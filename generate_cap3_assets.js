...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .wireflow-card { padding: 26px; }
  .wireflow-grid { display:grid; gap:24px; }
  .flow-row { display:grid; grid-template-columns: 184px 42px 184px 42px 184px 42px 184px 42px 184px; align-items:center; gap:8px; }
  .flow-row.short { grid-template-columns: 184px 42px 184px 42px 184px 42px 184px; }
  .screen-node,
  .decision-node,
  .state-node { min-height:94px; border-radius:8px; padding:14px; background:var(--white); border:2px solid var(--green); display:flex; flex-direction:column; justify-content:center; gap:7px; box-shadow:0 8px 18px rgba(22,33,31,.07); }
  .screen-node span,
  .decision-node span,
  .state-node span { font-size:11px; text-transform:uppercase; letter-spacing:.06em; color:var(--muted); font-weight:800; }
  .screen-node strong,
  .decision-node strong,
  .state-node strong { font-size:15px; line-height:1.25; }
  .screen-node p,
  .decision-node p,
  .state-node p { margin:0; color:var(--muted); font-size:12px; line-height:1.35; }
  .decision-node { width:126px; min-height:126px; justify-self:center; transform:rotate(45deg); border-color:var(--amber); background:#FFF8EB; }
  .decision-node > div { transform:rotate(-45deg); text-align:center; }
  .decision-node span { display:block; margin-bottom:4px; }
  .state-node { border-color:var(--blue); background:#F7FAFF; }
  .error-node { border-color:#EF4444; background:#FFF5F5; }
  .flow-note { color:var(--muted); font-size:13px; margin:18px 0 0; }
  .flow-branch { margin-top:18px; margin-left:226px; display:grid; grid-template-columns:184px 42px 184px; align-items:center; gap:8px; }
...
<section class="canvas" id="wireflows">
  ...
  <p class="sub">Flujos principales asociados a los objetivos de usuario del alcance inicial. Cada bloque representa una pantalla o estado de interfaz, incluyendo decisiones y caminos alternativos.</p>
  <div class="card wireflow-card">
    <div class="wireflow-grid">
      <div>
        <h3>Flujo 1: acceso seguro</h3>
        <div class="flow-row short">
          <div class="screen-node"><span>Pantalla</span><strong>Inicio</strong><p>Usuario entra a EcoCommute.</p></div>
          <div class="arrow">→</div>
          <div class="decision-node"><div><span>Decisión</span><strong>¿Tiene cuenta?</strong></div></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Login / Registro</strong><p>Ingresa datos y acepta validaciones.</p></div>
          <div class="arrow">→</div>
          <div class="state-node"><span>Estado</span><strong>Dashboard</strong><p>Accede a rutas, puntos y progreso.</p></div>
        </div>
        <div class="flow-branch">
          <div class="error-node screen-node"><span>Error</span><strong>Credenciales inválidas</strong><p>Se muestra alerta y permite reintentar.</p></div>
          <div class="arrow">↺</div>
          <div class="screen-node"><span>Pantalla</span><strong>Login / Registro</strong><p>Corrige correo o contraseña.</p></div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flow-row">
          <div class="screen-node"><span>Pantalla</span><strong>Buscar ruta</strong><p>Origen, destino y horario.</p></div>
          <div class="arrow">→</div>
          <div class="decision-node"><div><span>Decisión</span><strong>¿Hay ruta sostenible?</strong></div></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Elegir alternativa</strong><p>Bicicleta, caminata, bus o eléctrico.</p></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Registrar viaje</strong><p>Confirma distancia y medio usado.</p></div>
          <div class="arrow">→</div>
          <div class="state-node"><span>Estado</span><strong>CO₂ + puntos</strong><p>Actualiza EcoPerfil y dashboard.</p></div>
        </div>
        <p class="flow-note">Si no hay ruta sostenible disponible, el sistema muestra alternativas cercanas o permite registrar el viaje manualmente.</p>
      </div>

      <div>
        <h3>Flujo 3: recompensas y canje</h3>
        <div class="flow-row">
          <div class="screen-node"><span>Pantalla</span><strong>Recompensas</strong><p>Catálogo de beneficios activos.</p></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Detalle</strong><p>Puntos requeridos y tienda aliada.</p></div>
          <div class="arrow">→</div>
          <div class="decision-node"><div><span>Decisión</span><strong>¿Puntos suficientes?</strong></div></div>
          <div class="arrow">→</div>
          <div class="state-node"><span>Estado</span><strong>Canje exitoso</strong><p>Código generado y puntos descontados.</p></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Historial</strong><p>Consulta canjes y estado.</p></div>
        </div>
        <div class="flow-branch">
          <div class="error-node screen-node"><span>Error</span><strong>Puntos insuficientes</strong><p>Se informa saldo y viajes sugeridos.</p></div>
          <div class="arrow">→</div>
          <div class="screen-node"><span>Pantalla</span><strong>Buscar ruta</strong><p>Vuelve a generar puntos verdes.</p></div>
        </div>
      </div>
    </div>
  </div>
